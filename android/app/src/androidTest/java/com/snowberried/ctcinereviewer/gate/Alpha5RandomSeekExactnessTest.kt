...
class Alpha5RandomSeekExactnessTest {
    ...
    @Test
    fun deterministicRandomTargetsRemainExact() {
        assumeTrue(
            "Alpha 5 random exactness gate requires Samsung SM-S928*",
    ...
    private fun exerciseFixture(activity: GateActivity, spec: FixtureSpec): JSONObject {
        ...
        assertTrue("exactness probe did not perform full-frame readback", diagnostics.fullFrameReadbackCount > 0L)
    ...
    private fun validatePublished(golden: Golden, actual: FrameResult.Published) {
        ...
        val imageProbe = requireNotNull(actual.imageProbe) { "random exactness probe missing" }
        mismatchCount += if (imageProbe.embeddedFrameId != expected.embeddedFrameId) 1 else 0
        assertEquals(expected.imageSignature.size, imageProbe.imageSignature.size)
        val errors = imageProbe.imageSignature.zip(expected.imageSignature) { left, right ->
            abs(left - right)
        }.sorted()
        ...
        assertEquals(expected.embeddedFrameId, imageProbe.embeddedFrameId)
            ...
            frames = List(frames.length()) { index ->
                frames.getJSONObject(index).let { frame ->
                    GoldenFrame(
                        ...
                        imageSignature = List(frame.getJSONArray("imageSignature").length()) { valueIndex ->
                            frame.getJSONArray("imageSignature").getInt(valueIndex)
                        },
                    )
                }
            },
    ...
    private fun writeReport(outcome: Result<Unit>, fixtures: JSONArray) {
        ...
        val report = JSONObject()
            ...
            .put("kind", "alpha5-cost-aware-random-exactness")
            .put("status", if (outcome.isSuccess) "PASS" else "FAIL")
            .put("baselineCompleteness", "EXACTNESS_COMPLETE")
            .put("renderMode", "EXACTNESS")
    ...
    companion object {
        const val REPORT_FILE = "s24-alpha5-random-exactness-v1.json"
