...
v0.5.6은 MP4 화면 보정 프리셋을 제거해 수동 미세 조정 흐름으로 단순화하고, 빈 화면 파일 열기·빠른 프레임 이동 간격·사용자 지정 단축키를 추가한 로컬 Windows 빌드이다. Electron IPC, 디코더, cache와 주석 데이터 모델은 변경하지 않았다.
...
- 크기: 143,382,041 bytes
- SHA-256: `c8e6d36c726499622d2209cef5fe172fedb2702c61fbfcc110dd1cfef45a9009`
- NSIS x64, 현재 사용자 설치, 관리자 권한 요구 없음

`artifacts/`는 Git 배포 대상이 아니다. 공개 Release가 필요하면 별도 승인 후 `v0.5.6` 태그를 생성해 기존 Windows Release 워크플로를 실행한다.
