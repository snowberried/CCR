...
실제 파일은 아래 기술 범주를 가능한 한 하나씩 준비하되, 파일명·경로·hash는 이 문서나 tracked report에 남기지 않는다.

- 720p H.264 8-bit
- 1080p H.264 8-bit
- HEVC Main 8
- B-frame 또는 long-GOP H.264
- VFR 또는 rotation metadata가 있는 영상
...
- [ ] −5, −1, +1, +5 네 버튼을 각각 길게 누르고 방향·반복·종료를 확인한다.
- [ ] 길게 누르다 손을 떼거나 취소한 뒤 requested frame overshoot가 0이다.
- [ ] 화면의 frame 숫자 변화와 실제 영상 변화가 같은 흐름으로 느껴지며 건너뛰는 인상이 없는지 기록한다.
...
- [ ] 검정/녹색/공백 화면, 잘못된 rotation, 잘못된 프레임 또는 crash가 있으면 비식별 재현 단계만 기록한다.

문제를 보고할 때는 실제 파일명·경로·화면 캡처 대신 앱의 `진단 복사` 결과와 비식별 동작 순서만 사용한다.
