...
## v0.5.2 사용자 승인 확대율 UI

이 문서 아래의 Phase 3B 당시 Fit 대비 step과 별도 100% command 기록은 역사적 검증 결과로 유지한다. 현재 제품 UI는 다음 계약으로 대체한다.

- 확대율 셀은 원본 픽셀 기준 현재 실제 `%`를 계속 표시한다.
- 클릭 시 `화면 맞춤, 50%, 75%, 100%, 125%, 150%, 175%, 200%`를 선택한다.
- `−/+`, 키보드와 Ctrl+wheel step은 실제 배율 기준 10%p다.
- 별도 화면 맞춤 버튼은 제거하고 화면 맞춤을 선택 메뉴 안에 둔다.
- 화면 맞춤 선택 뒤에도 셀 문구는 `화면 맞춤`이 아니라 계산된 현재 `%`다.
