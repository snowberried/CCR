...
## 향후 검토: 모바일 앱 (Android 우선)

### 범위와 진행 조건

- 현재 병원 미니 PC에서는 모바일 개발·빌드·무선 디버깅을 진행하지 않는다.
- 모바일 개발은 추후 집 데스크탑에서 별도 사용자 승인 후 시작한다.
- Android를 우선 구현하되, 향후 필요하면 iPhone을 추가할 수 있도록 공통 코어와 플랫폼 전용 구현의 경계를 유지한다.
- 현재 프로젝트 전체를 복사하지 않고 같은 Git 저장소에 모바일 앱을 별도 구성한다. 기존 Electron 데스크탑 앱은 초기 모바일 검증 단계에서 이동하거나 재구성하지 않는다.

### 초기 기술 검증

- 실제 PACS MP4의 로컬 파일 열기와 개인정보 비전송 원칙
- 정확한 frame index·PTS, 첫/중간/마지막 프레임과 앞뒤 한 프레임 이동
- 빠른 탐색·역방향 이동, 화면 보정, 메모리·속도·발열
- Android 네이티브 영상 처리 adapter를 검증하고, iPhone 확장 시 동일한 공통 계약 아래 별도 adapter를 추가한다.
- 모바일 UI는 터치·화면 크기·회전에 맞게 별도 구현하고, 프레임 규칙·좌표·보정 모델만 공통 코어로 재사용한다.

### 다음 단계 승인

- 테스트 Android 기종·버전과 대표 MP4 선정
- 모바일 기술 스택과 공통 코어 분리 범위
- 최소 검증판 결과를 확인한 뒤 정식 모바일 앱 진행 여부
