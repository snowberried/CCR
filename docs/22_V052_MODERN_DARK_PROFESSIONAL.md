...
- 화면 보정의 Original reset은 상단 `초기 설정` 하나만 제공하며, 중복 `원본` 버튼은 두지 않는다. 프리셋 `원본`과 일시적인 `원본 비교(O)`는 유지한다.
...
- 하단 왼쪽에는 독립 설정창을 여는 테두리 없는 13px `설정`을 둔다. 설정창은 사용자가 직접 실행하는 GitHub 최신 Release 확인 하나만 제공한다. 3:1:3 탐색은 두 유틸리티 사이의 중앙에 놓고 `#FFD54F` 2px·80% opacity 구분선은 양쪽 빈 여백의 중앙에 둔다.
