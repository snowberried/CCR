...
Phase 2 최소 실사용 뷰어와 Phase 2.1 Windows 패키징 구현은 완료했다. `win-unpacked` 독립 실행 검증과 NSIS Setup 생성·개인정보 검사는 통과했다.

**실제 Windows 설치, 제거, 재설치는 아직 시행하지 않았다.** 설치 직전 사용자 확인이 시간 초과됐고, 사용자가 여기서 중단하고 인수인계·커밋하도록 지시했다. 다음 작업은 제품 기능 추가가 아니라 설치형 파일럿 검증을 이어가는 것이다.
...
- Windows 제거 등록의 `CT Cine Reviewer` 항목 0건: 실제 설치 미시행 상태 확인
...
다음 작업에서는 **새 기능을 구현하지 말고** 아래 순서로 Phase 2.1을 끝낸다.

1. `AGENTS.md`, 이 문서, `docs/10_PHASE2_MINIMUM_VIEWER.md`를 읽는다.
2. `npm test`, `npm run build`, `npm run verify:package`로 기준선을 확인한다.
3. 사용자의 설치 직전 확인을 받은 뒤 Setup EXE를 실행한다.
4. 현재 사용자 설치, 설치 위치, 시작 메뉴, 바탕화면, 제거 등록을 확인한다.
5. 설치본에서 Sample A/B/C, 실제 Windows drag/drop, Ctrl+O와 모든 탐색 조작을 확인한다.
6. 공백·한글·특수문자·긴 경로·읽기 전용·다른 드라이브 경로를 비의료 synthetic 영상으로 확인한다.
7. 첫 프레임, hit/miss, 먼 seek, 빠른 휠, A→B→C, 취소, 메모리 상한의 설치본 회귀를 측정한다.
8. 앱 종료 후 Electron/ffmpeg/ffprobe와 임시 frame 파일이 남지 않는지 확인한다.
9. 설치 중 외부 다운로드가 없고 앱 네트워크 요청이 없는지 확인 가능한 범위와 한계를 기록한다.
10. 제거, 원본 MP4 보존, 바로가기·등록·FFmpeg·라이선스 제거, 재설치와 동일 버전 덮어쓰기를 확인한다.
11. `docs/11_PHASE2_1_WINDOWS_INSTALLER.md`에 최종 결과를 작성한다.
...
- 실제 설치 상태의 GUI drag/drop, 경로 matrix, 네트워크 관찰, 제거·재설치는 전부 미완료다.
