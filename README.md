...
Phase 2.3 I420 제품 cache부터 Phase 4B-1 PNG/clipboard까지의 기준선 위에 Phase 5 **비교 보기**를 통합했다. 뷰 A와 뷰 B는 항상 같은 영상의 동일한 현재 프레임을 공유하고, Zoom/Pan과 화면 보정은 pane별로 독립적이다. 연결 십자선은 DICOM registration이 아니라 같은 image pixel 좌표를 두 transform 사이에서 대응시킨다. v0.5.2는 이 기능 기준선을 유지한 채 Modern Dark Professional UI, 시각적 타임라인, 조정·정보 분리 컬럼과 대칭 프레임 탐색을 적용한다. 상세 내용은 [Phase 5](docs/19_PHASE5_LINKED_DUAL_VIEW.md), [v0.5.1 UI Polish](docs/20_V051_UI_POLISH.md), [Frame Navigation & Panel Separation](docs/21_FRAME_NAVIGATION_LAYOUT.md)과 [v0.5.2 Modern Dark Professional](docs/22_V052_MODERN_DARK_PROFESSIONAL.md)에서 확인한다.
...
- 동일 frame/pixels를 공유하는 뷰 A/B 비교 보기, pane별 독립 View/Display/tool과 image-space 연결 십자선 구현
- `MM:SS.mmm / MM:SS.mmm` 시간 표시와 진행 구간·marker·playhead가 있는 프레임 타임라인 구현
- 오른쪽 화면/주석/내보내기 조정 컬럼과 영상/진단 정보 컬럼 분리
- 첫·−5·이전 1 / 현재 프레임 / 다음 1·+5·마지막의 3:1:3 대칭 탐색 구현
